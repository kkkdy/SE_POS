...
public class SalesInputService implements KeyListener, ActionListener {
	...
	Vector<PosDto> salesList;//import한 부분
...
//		if()유저이름이 없다면 멤버쉽등록이 안됬으므로 포인트처리 생략
	...
	public void listAdd(Vector<com.kitri.pos.db.PosDto> vector) {
		if (vector == null)
			...
			rows.addElement(Integer.toString(vector.get(0).getListNum()));
			rows.addElement(vector.get(0).getProductCode());
			rows.addElement(vector.get(0).getProductName());
			rows.addElement(Integer.toString(vector.get(0).getPrice()));
			rows.addElement(String.valueOf(vector.get(0).getSellCount()));
			rows.addElement(String.valueOf(vector.get(0).getPricensellCount()));
			rows.addElement(vector.get(0).getInDate());
